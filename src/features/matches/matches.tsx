      ...
      {matchesData && teamsData && (
        <div className="space-y-4">
          {matchesData.matches.map(match => {
            ...
            const otherTeam = match.players.filter(
              player => player.team_id !== teamId,
            )
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <h3 className="text-base font-bold flex items-center gap-2 pb-2 border-b border-slate-700">
                      ...
                      {scoutedTeamWon && (
                        <span className="text-xs px-2 py-1 bg-green-500/20 text-green-400 rounded-full border border-green-500/30 flex-shrink-0">
                          ✓ W
                        </span>
                      )}
                      {otherTeamWon && (
                        <span className="text-xs px-2 py-1 bg-red-500/20 text-red-400 rounded-full border border-red-500/30 flex-shrink-0">
                          ✗ L
                        </span>
                      )}
                    </h3>
                  ...
                  <div className="space-y-2">
                    <h3 className="text-base font-bold flex items-center gap-2 pb-2 border-b border-slate-700">
                      <span className="truncate" title={otherTeamName}>
                        {otherTeamName}
                      </span>
                      {otherTeamWon && (
                        <span className="text-xs px-2 py-1 bg-green-500/20 text-green-400 rounded-full border border-green-500/30 flex-shrink-0">
